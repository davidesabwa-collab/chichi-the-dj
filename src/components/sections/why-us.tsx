...
import type { WhyUsContent } from '@/types/site-content';
...
export default async function WhyUs() {
    const content = (await getSiteContent<WhyUsContent>('why-us')) || defaultContent;
                        ...
                        <div key={index} className="text-center p-6 border border-gray-800 rounded-lg hover:bg-gray-900/50 hover:border-primary/30 transition-all duration-300">
                           <div className="flex justify-center mb-4">
                             <div className="bg-primary/10 p-4 rounded-full">
                                <SiteIcon name={item.icon} className="h-8 w-8 text-primary" />
                             </div>
                           </div>
